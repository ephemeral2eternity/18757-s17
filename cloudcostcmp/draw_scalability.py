...
if __name__ == "__main__":
    ...
    data_file = "scalability-noakamai.csv"
    ...
    g = sns.factorplot(x="User demand", y="Session crashes", hue="Provider", data=df, col="Region", kind="bar",
                       palette="muted", ci=95,
                       n_boot=1000, size=6.5, aspect=1, sharex=True)

    g.despine(left=True)
    g.set_ylabels("Number of Session Crashes")
    g.set_xlabels("User demand")
    g.set_xticklabels([r'256', r'512', r'1024'])
    g._legend.set_title('Provider')
    plt.show()
    g.savefig("scalability_session_crashes_noakamai.pdf")
    ...
    g._legend.set_title('Provider')
    plt.show()
    g.savefig("scalability_QoE_noakamai.pdf")
