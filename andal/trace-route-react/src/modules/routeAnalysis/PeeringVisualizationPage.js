//Reference: https://jsfiddle.net/sravanrekandar/4pygu0qo/
//           https://bl.ocks.org/mbostock/1046712

import React, {Component} from 'react'
import d3 from 'd3'
import _ from 'lodash'
import RAW_DATA from './assets/sample'
import AS_DATA from './assets/As_Isps'
import './peeringVisualization.css'

function prepareMatrix(uniqueKeys, data) {
  const matrix = []
  uniqueKeys.forEach((row) => {
    const rowSet = []
    uniqueKeys.forEach(col => {
      let colValue = 0
      if (row !== col) {

        const combinationsRowToCol = data.filter(r => (r.AS === row && (r.neighborAS === col)))
        const combinationsColToRow = data.filter(r => (r.AS === col && (r.neighborAS === row) ))
        colValue = combinationsColToRow.length + combinationsRowToCol.length
      }
...
let keys = RAW_DATA.map(e => [e.AS, e.neighborAS])
...
export default class PeeringVisualization extends Component {
  ...
  renderGraph(container){
    ...
    function ready(groups, matrix) {
      ...
      group.append("title").text(function (d, i) {
        ...
        const ispName = AS_DATA.filter(function (el) {
          if(el.AS === groups[i].name) {
            return el.ISP
          }
        })
        return groups[i].name + ": " + (d.value) + " connections, ISP: " + ispName[0].ISP
      })
  ...
  render() {
    return (
      <div>
        <h1>Peering Visualization (Clients, transit, Cloud)</h1>
        <div ref="container"/>
      </div>
    )
  }
